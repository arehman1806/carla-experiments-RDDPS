...
class StandStillAgent(object):
    ...
    def _detect_opposite_direction_vehicles(self, vehicle_list=None):
        ...
        if not vehicle_list:
            vehicle_list = self._world.get_actors().filter("*vehicle*")
        vehicle_list = [vehicle for vehicle in vehicle_list if vehicle.attributes.get("role_name") != "hero"]
        ...
        for target_vehicle in vehicle_list:
            ...
            heading = np.arctan2(target_location.y - ego_location.y, target_location.x - ego_location.x)
            #TODO properly set this threshold through trial and error
            if heading > -1.8 and distance < 30:
                vehicle_count += 1
                if distance < distance_to_ego:

...
# cost_points = [0, 50]
s_grid = RectangleGrid(ds_ego, vs_ego, ds_actor)
𝒮 = [[d_ego, v_ego, d_actor] for d_ego in ds_ego, v_ego in vs_ego, d_actor in ds_actor];
s2pt(s) = s
...
# Plot one sample
display(heatmap(ds_ego, ds_actor, (d_ego, d_actor) -> CVaR([d_ego, 15, d_actor], [0], 0.7), title="α = 0, ego_vel = 25", xlabel="ego displacement from junction (m)", ylabel="actor displacement from junction (m)"))# plot(all_distance_actor, (y) -> CVaR([20, y], [0], 0.0), title="α = 0", xlabel="actor_distance_junction (m)", ylabel="Risk")
...
#     heatmap(ds_ego, ds_actor, (x, y) -> CVaR([x, 10, y], [0], α), title="CVaR (α = $α)", xlabel="distance ego (m)", ylabel="distance actor (m)")
# end
